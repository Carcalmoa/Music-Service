...
public class DataModel extends HttpServlet{
    ...
    ArrayList<String> errorsMUML = new ArrayList<String>(); //LIST OF MUMLS WITH ERRORS FOR THE AUTO MODE SRCEEN
    ArrayList<String> errorsLink = new ArrayList<String>(); //LIST OF MUMLS WITH ERRORS FOR THE BROWSER MODE SRCEEN
    ArrayList<String> fatalErrorsMUML = new ArrayList<String>();    //LIST OF MUMLS WITH FATAL ERRORS FOR THE AUTO MODE SRCEEN
    ArrayList<String> fatalErrorsLink = new ArrayList<String>();    //LIST OF MUMLS WITH FATAL ERRORS FOR THE BROWSER MODE SRCEEN
    ArrayList<String> correctLinks = new ArrayList<String>();   //LIST OF THE CORRECT MUMLS TO USE IT IN THE GETQ1 FUNCTIONS

    Map <Integer,Document > correctMumls = new HashMap<Integer, Document>();

    //GETTERS FOR HAVING THE LISTS IN SINT175P2
    public ArrayList<String> getErrorsMUML(){
        return errorsMUML;
    }
    public ArrayList<String> getErrorsLink(){
        return errorsLink;
    }
    public ArrayList<String> getFatalErrorsMUML(){
        return fatalErrorsMUML;
    }
    public ArrayList<String> getFatalErrorsLink(){
        return fatalErrorsLink;
    }
    public ArrayList<String> getCorrectMUML(){
        return correctLinks;
    }
    ...
    //CLASS FOR SETTING THE SORT FOR THE ALBUMS_LIST
    class CompareAlbums implements Comparator<album>{
        public int compare(album a1, album a2){
            if (a1.getYear() < a2.getYear()) return -1;
    ...
    //CLASS FOR SETTING THE SORT FOR THE SONGS_LIST
    class CompareSongs implements Comparator<song>{
        public int compare(song s1, song s2){
            if (s1.getGenres().size() < s2.getGenres().size()) return -1;
            else if (s1.getGenres().size()==s2.getGenres().size())
    ...
    public void errorChecker() throws SAXException, ParserConfigurationException, IOException, MalformedURLException, XPathExpressionException{ //METHOD FOR GETTING ALL THE MUMLS AND ANALIZE IF THEY ARE CORRECT OR IF THEY HAVE ERRORS OR FATALERRORS
        ...
        while(mumls_to_analyze_list.size()!=0){ 
            ...
            try{
                ...
                //CORRECT MUML
                    ...
                    correctLinks.add(linkString); //SAVE THE LINK WITH THE CORRECT MUML FOR THE GETQ1 FUNCTIONS
                    ...
                    for(int i=0; i<nl_muml.getLength();i++){    //CHECK THE LIST WITH ALL THE MUMLS OF THE LINK
                        ...
                        mumlString = elem_muml.getTextContent().trim();
...
//METHOD TO CREATE AND FILL THE COUNTRIES LIST WITH ALL THE COUNTRIES THAT APPEAR IN THE CORRECT MUMLS
    public ArrayList<String> getQ1Countries() throws SAXException, ParserConfigurationException, IOException, MalformedURLException{    
        
        ArrayList<String> countries = new ArrayList<String>();

        // GO THROUGH THE MAP TO GET DOCUMENT BY DOCUMENT(CORRECT MUML)
...
//METHOD TO CREATE AND FILL THE LIST OF ALBUMS WITH THE ALBUMS THAT APPEAR IN THE CORRECT MUMLS THAT SHARE A SPECIFIC COUNTRY VALUE RECIVED AS ARGUMENT
    public ArrayList<album> getQ1Albums (String country) throws SAXException, ParserConfigurationException, IOException, MalformedURLException, XPathExpressionException{  
        ...
        listAlbums.sort(new CompareAlbums());
        return listAlbums;
    }
...
//METHOD TO CREATE AND FILL THE LIST OF SONGS WITH THE SONGS THAT APPEAR IN THE CORRECT MUMLS THAT SHARE A SPECIFIC COUNTRY AND ALBUM AID, BOTH PASSED BY ARGUMENTS
    public ArrayList<song> getQ1Songs (String country, String album) throws SAXException, ParserConfigurationException, IOException, MalformedURLException, XPathExpressionException{
        ...
        for (int key:correctMumls.keySet()){
            ...
            for(int i=0; i<nl_song.getLength();i++){
                ...
                if (genre_list.contains("Pop")){
                     //CREATE THE SONG AND ADD IT TO THE LIST OF SONGS
                    song song= new song(songTitle, songLanguage, genre_list, songComposer );
                    listSongs.add(song);
                }              
            }
        }
        //SORT THE LIST OF SONGS
        listSongs.sort(new CompareSongs());
        return listSongs;
    }
